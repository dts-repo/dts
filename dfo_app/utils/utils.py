...
def validate_page(page,session):
    valid=1
    if page=="integrated_plot_plot1":
        if session["integrated_plot_data"]=="DTS":
            if not (os.path.exists(os.path.join(session["output_dts_folder"],"dts.csv")) and \
                os.path.exists(os.path.join(session["output_dts_folder"],"depths.txt")) and \
                os.path.exists(os.path.join(session["output_dts_folder"],"timestamps.txt"))):
                valid=0
        else:
            if not (os.path.exists(os.path.join(session["norm_dts_folder"],"dts.csv")) and \
                os.path.exists(os.path.join(session["norm_dts_folder"],"depths.txt")) and \
                os.path.exists(os.path.join(session["norm_dts_folder"],"timestamps.txt"))):
                valid=0
        if not os.path.exists(os.path.join(dirname,r"temp\completion.json")):
            valid=0
    elif page=="geothermal":
        if not (os.path.exists(os.path.join(session["output_dts_folder"],"dts.csv")) and \
            os.path.exists(os.path.join(session["output_dts_folder"],"depths.txt")) and \
            os.path.exists(os.path.join(session["output_dts_folder"],"timestamps.txt"))):
            valid=0
